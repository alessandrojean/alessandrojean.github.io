<template>
  <blockquote data-slot="quote">
    <p>
      <BlockTextRenderer :rich-text="block.quote.rich_text" :id-map />
    </p>

    <BlockRenderer v-if="children?.length" :blocks="children" :id-map />
    
    <cite v-if="cite" data-slot="quote-cite">
      <BlockTextRenderer :rich-text="cite.paragraph.rich_text" :id-map />
    </cite>
  </blockquote>
</template>
