<template>
  ...
  >
    ...
    >
      <Icon v-if="item.to_do.checked" name="lucide:square-check-big" class="text-sky-700 dark:text-sky-500" />
      <Icon v-else name="lucide:square" />
      <span v-if="item.to_do.checked" class="sr-only">Concluído: </span>
      <BlockTextRenderer :rich-text="item.to_do.rich_text" :id-map />
    </li>
  </ul>
</template>
